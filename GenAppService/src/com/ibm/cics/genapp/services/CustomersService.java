...
import com.ibm.cics.genapp.bli.*;
...
public class CustomersService {
    ...
    @javax.ws.rs.GET
    @javax.ws.rs.Path("/{customerNumber}/insurance")
    @javax.ws.rs.Produces("application/json")
    public JSONObject fetchInsuranceData(@javax.ws.rs.PathParam("customerNumber") int customerNumber) {
        ...
        // Add some dummy data
        JSONObject pJson = new JSONObject();
        pJson.put("time", "02.01.1970 - 09.15 Uhr");
        pJson.put("data", "Bleeding right knee with some dirt in it.");
        jsonArray.add(pJson);

        pJson = new JSONObject();
        pJson.put("time", "01.01.1970 - 08.25 Uhr");
        pJson.put("data", "Broken finger on left hand.");
        jsonArray.add(pJson);

        jsonResponse.put("insurance", jsonArray);
